...
class Hyperparameters:
    ...
    # --- Track C: Eval-time experiments ---
    ttt_control_only = bool(int(os.environ.get("TTT_CONTROL_ONLY", "0")))  # E33
    int8_mlp_down = bool(int(os.environ.get("INT8_MLP_DOWN", "0")))  # E17
    ngram_cache_enabled = bool(int(os.environ.get("NGRAM_CACHE", "0")))  # E23
    ngram_cache_order = int(os.environ.get("NGRAM_CACHE_ORDER", 5))
    ngram_cache_lambda = float(os.environ.get("NGRAM_CACHE_LAMBDA", 0.1))
    ngram_cache_adaptive = bool(int(os.environ.get("NGRAM_CACHE_ADAPTIVE", "1")))
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_control_only:
        # E33: Only update control tensors (gates, scales, VE weights, smear)
        ttt_params = []
        ...
        log0(f"ttt_sliding:control_only params={sum(p.numel() for p in ttt_params)} "
             f"names={[n for n, p in base_model.named_parameters() if p.requires_grad]}")
    else:
        # Standard: freeze first N blocks, unfreeze everything else
...
# --- E23: N-gram cache with entropy-adaptive interpolation ---

class NgramCache:
    """Online n-gram language model built from scored tokens."""
    def __init__(self, max_order: int = 5, vocab_size: int = 1024):
        self.max_order = max_order
        self.vocab_size = vocab_size
        # counts[n][(ctx_tuple)] = count_array[vocab_size]
        self.counts: list[dict] = [{} for _ in range(max_order + 1)]
        self.total_tokens = 0

    def update(self, tokens: list[int] | np.ndarray) -> None:
            ...
            for n in range(1, self.max_order + 1):
                if i >= n - 1:
                    ...
                    if ctx not in self.counts[n]:
                        self.counts[n][ctx] = np.zeros(self.vocab_size, dtype=np.float32)
                    self.counts[n][ctx][tok] += 1

    def log_probs(self, context: list[int]) -> np.ndarray | None:
        """Get log-probability distribution using backoff from highest to lowest order.
        Returns None if no matching context found at any order."""
        for n in range(self.max_order, 0, -1):
            ...
            if ctx in self.counts[n]:
                ...
                total = counts.sum()
                if total < 2:  # need minimum observations
                    continue
                # Add-k smoothing (k=1)
                smoothed = (counts + 1.0) / (total + self.vocab_size)
                return np.log(smoothed)
        return None


def eval_val_sliding_ngram_cache(
    args: "Hyperparameters", base_model: nn.Module, rank: int, world_size: int,
    device: torch.device, val_tokens: Tensor, base_bytes_lut: Tensor,
    has_leading_space_lut: Tensor, is_boundary_token_lut: Tensor,
    stride: int, batch_seqs: int = 32, log0=print,
) -> tuple[float, float]:
    """Score-first eval with n-gram cache interpolation (E23).
    Process chunks sequentially: score chunk → add scored tokens to cache → next chunk.
    Neural logits interpolated with cache: log p = (1-λ) * log p_nn + λ * log p_cache."""
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    chunk_size = args.ttt_chunk_tokens  # reuse TTT chunk size
    num_chunks = (total_tokens + chunk_size - 1) // chunk_size

    cache = NgramCache(max_order=args.ngram_cache_order, vocab_size=args.vocab_size)
    base_lambda = args.ngram_cache_lambda
    adaptive = args.ngram_cache_adaptive

    log0(f"ngram_cache:start order={args.ngram_cache_order} lambda={base_lambda} "
         f"adaptive={adaptive} chunks={num_chunks} chunk_tokens={chunk_size}")

    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)

    # Window starts for sliding window eval
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= stride or ws == 0]
    ...
    val_np = val_tokens.numpy().astype(int)
    base_model.eval()
    t0 = time.perf_counter()
    ...
    for ci in range(num_chunks):
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                # Interpolate with cache if available
                log_probs_nn = F.log_softmax(logits.float(), dim=-1)  # [bsz, seq_len, V]

                if cache.total_tokens > 0:
                    # Apply cache interpolation per-token
                    for i, ws in enumerate(batch_ws):
                        wlen = wlens[i]
                        s_pos = 0 if ws == 0 else max(wlen - stride, 0)
                        for t_pos in range(s_pos, wlen):
                            global_pos = ws + t_pos
                            # Get context for this token
                            ctx_start = max(0, global_pos - args.ngram_cache_order + 1)
                            ctx = val_np[ctx_start:global_pos].tolist()
                            cache_lp = cache.log_probs(ctx)
                            if cache_lp is not None:
                                lam = base_lambda
                                if adaptive:
                                    # Entropy-adaptive: high model entropy → lean more on cache
                                    nn_probs = log_probs_nn[i, t_pos].exp()
                                    entropy = -(nn_probs * log_probs_nn[i, t_pos]).sum().item()
                                    max_entropy = math.log(args.vocab_size)
                                    entropy_ratio = entropy / max_entropy
                                    lam = base_lambda * (entropy_ratio ** 0.5)
                                cache_lp_t = torch.tensor(cache_lp, device=device, dtype=torch.float32)
                                log_probs_nn[i, t_pos] = (
                                    torch.logaddexp(
                                        log_probs_nn[i, t_pos] + math.log(1 - lam),
                                        cache_lp_t + math.log(max(lam, 1e-10)),
                                    )
                                )

                # Compute NLL from (potentially interpolated) log probs
                nll = -log_probs_nn.gather(
                    -1, y_batch.unsqueeze(-1).long()
                ).squeeze(-1)  # [bsz, seq_len]

                for i, ws in enumerate(batch_ws):
                    wlen = wlens[i]
                    s_pos = 0 if ws == 0 else max(wlen - stride, 0)
                    scored_nll = nll[i, s_pos:wlen].to(torch.float64)
                    loss_sum += scored_nll.sum()
                    token_count += float(wlen - s_pos)
                    tgt, prev = y_batch[i, s_pos:wlen], x_batch[i, s_pos:wlen]
                    tb = base_bytes_lut[tgt].to(torch.float64)
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()

        # Add scored tokens to cache (after scoring — legal)
        chunk_start = ci * chunk_size
        chunk_end = min((ci + 1) * chunk_size, total_tokens)
        cache.update(val_np[chunk_start:chunk_end + 1])

        if rank == 0 and (ci % 50 == 0 or ci == num_chunks - 1):
            elapsed = time.perf_counter() - t0
            rl = loss_sum.item() / max(token_count.item(), 1)
            rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1))
            log0(f"  ngram_chunk [{ci+1}/{num_chunks}] bpb={rbpb:.6f} cache_tokens={cache.total_tokens:,} time={elapsed:.1f}s")
    ...
    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    log0(f"ngram_cache:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
         f"elapsed={time.perf_counter() - t0:.1f}s")
    return val_loss, val_bpb
...
def _classify_param_e17(name: str) -> str:
    """E17: separate mlp_down from mlp_up for mixed quantization."""
    if "tok_emb" in name or "lm_head" in name:
        return "embed"
    if ".mlp.proj." in name:  # mlp down-projection
        return "mlp_down"
    if ".mlp." in name:  # mlp up-projection
        return "mlp"
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
                        classify_fn=None, int8_override_cats: set[str] | None = None):
    classify_fn = classify_fn or _classify_param
    int8_override_cats = int8_override_cats or set()
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        # E17: int8_override_cats get int8 instead of int6
        if cat in int8_override_cats and t.ndim >= 1:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
...
def main() -> None:
    ...
    # E17: use int8 for mlp_down, int6 for everything else
    if args.int8_mlp_down:
        quant_result, quant_meta = mixed_quantize_int6(
            unbanked_sd, {"mlp", "attn"}, classify_fn=_classify_param_e17,
            int8_override_cats={"mlp_down"})
    else:
    ...
    # E23: N-gram cache evaluation
    if args.ngram_cache_enabled:
        # Reload fresh model (TTT may have mutated weights)
        if args.ttt_enabled:
            eval_model.load_state_dict(deq_state, strict=True)
        torch.cuda.synchronize()
        t_ngram = time.perf_counter()
        ngram_loss, ngram_bpb = eval_val_sliding_ngram_cache(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, log0=log0,
        )
        torch.cuda.synchronize()
        log0(f"ngram_cache val_loss:{ngram_loss:.4f} val_bpb:{ngram_bpb:.4f} "
             f"eval_time:{1000.0 * (time.perf_counter() - t_ngram):.0f}ms")
        log0(f"ngram_cache_exact val_loss:{ngram_loss:.8f} val_bpb:{ngram_bpb:.8f}")
    if distributed:
