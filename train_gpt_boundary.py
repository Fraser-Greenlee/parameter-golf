...
class Hyperparameters:
    ...
    boundary_features = bool(int(os.environ.get("BOUNDARY_FEATURES", "0")))
    # --- Lookahead features ---
...
class GPT(nn.Module):
    def __init__(
        ...
        if boundary_features:
            # 4 boundary types: [non-boundary, has_leading_space, is_boundary_token, both]
            self.boundary_emb = nn.Embedding(4, model_dim)
            nn.init.zeros_(self.boundary_emb.weight)
            self.boundary_scale = nn.Parameter(torch.tensor(0.05, dtype=torch.float32))
            # LUT will be set after model creation via set_boundary_lut()
            self.register_buffer("boundary_lut", torch.zeros(vocab_size, dtype=torch.long), persistent=False)
    ...
    def set_boundary_lut(self, has_leading_space: Tensor, is_boundary_token: Tensor) -> None:
        """Precompute per-token boundary category from SentencePiece LUTs."""
        self.boundary_lut = (has_leading_space.long() + 2 * is_boundary_token.long()).to(self.boundary_lut.device)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor,
                ...
                boundary_ids: Tensor | None = None) -> Tensor:
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        if self.boundary_features:
            bids = boundary_ids if boundary_ids is not None else self.boundary_lut[input_ids]
            x = x + self.boundary_scale.to(dtype=x.dtype) * self.boundary_emb(bids)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        boundary_features=args.boundary_features,
    ).to(device).bfloat16()
    if args.boundary_features:
        base_model.set_boundary_lut(has_leading_space_lut, is_boundary_token_lut)
    ...
    if args.boundary_features:
        scalar_params.append(base_model.boundary_scale)
    ...
    if args.boundary_features:
        tok_params.append({"params": [base_model.boundary_emb.weight], "lr": token_lr, "base_lr": token_lr})
    ...
    if args.boundary_features:
        eval_model.set_boundary_lut(has_leading_space_lut, is_boundary_token_lut)
