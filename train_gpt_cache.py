...
class Hyperparameters:
    ...
    cache_lambda = float(os.environ.get("CACHE_LAMBDA", 0.1))
    cache_smoothing = float(os.environ.get("CACHE_SMOOTHING", 0.01))
...
class SmearGate(nn.Module):
    def __init__(self, dim: int, lookahead: bool = False):
        ...
        if lookahead:
            self.lookahead_scale = nn.Parameter(torch.tensor(0.1, dtype=torch.float32))
    def forward(self, x: Tensor, lookahead_emb: Tensor | None = None) -> Tensor:
        ...
        if self.lookahead and lookahead_emb is not None:
            out = out + self.lookahead_scale.to(dtype=out.dtype) * lookahead_emb
        return out
...
def eval_val_sliding_lookahead(
    args: "Hyperparameters",
    base_model: nn.Module,
    rank: int,
    world_size: int,
...
def eval_val_sliding_cache(
    ...
    """Sliding window eval with online bigram cache interpolation.
    Processes chunks sequentially; after scoring each chunk, updates a bigram
    count table. Neural logits are interpolated with cache log-probs."""
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    V = args.vocab_size
    cache_lambda = args.cache_lambda
    smoothing = args.cache_smoothing

    # Bigram count table: counts[prev_token, next_token]
    bigram_counts = torch.zeros(V, V, device=device, dtype=torch.float32)
    unigram_counts = torch.zeros(V, device=device, dtype=torch.float32)

    # Process sequentially in chunks for cache updates
    chunk_size = args.ttt_chunk_tokens if args.ttt_chunk_tokens > 0 else 32768
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= stride or ws == 0]
    ...
    log0(f"cache_eval:start chunks={num_chunks} lambda={cache_lambda} smoothing={smoothing}")
    ...
    for ci in range(num_chunks):
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                # Interpolate with cache if we have counts
                if cache_lambda > 0 and unigram_counts.sum() > 0:
                    # Compute cache log-probs from bigram counts
                    # For each position, look up bigram_counts[prev_token, :]
                    prev_tokens = x_batch  # [B, T]
                    cache_counts = bigram_counts[prev_tokens]  # [B, T, V]
                    cache_probs = (cache_counts + smoothing) / (cache_counts.sum(dim=-1, keepdim=True) + smoothing * V)
                    cache_log_probs = torch.log(cache_probs + 1e-10)
                    nn_log_probs = F.log_softmax(logits.float(), dim=-1)
                    mixed = (1 - cache_lambda) * nn_log_probs + cache_lambda * cache_log_probs
                    nll = F.nll_loss(
                        mixed.reshape(-1, V),
                        y_batch.reshape(-1),
                        reduction="none",
        ...
        # Update cache with this chunk's tokens (already scored = legal)
        chunk_start = ci * chunk_size
        chunk_end = min((ci + 1) * chunk_size + 1, val_tokens.numel())
        chunk_toks = val_tokens[chunk_start:chunk_end].to(device=device, dtype=torch.long)
        if chunk_toks.numel() > 1:
            prev_t = chunk_toks[:-1]
            next_t = chunk_toks[1:]
            # Scatter-add to bigram counts
            indices = prev_t * V + next_t
            bigram_counts.view(-1).scatter_add_(0, indices, torch.ones_like(indices, dtype=torch.float32))
            unigram_counts.scatter_add_(0, prev_t, torch.ones_like(prev_t, dtype=torch.float32))

        if rank == 0 and (ci % 50 == 0 or ci == num_chunks - 1):
            elapsed = time.perf_counter() - t0
            rl = loss_sum.item() / max(token_count.item(), 1)
            rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1)) if token_count.item() > 0 else 0.0
            log0(f"  cache_chunk [{ci+1}/{num_chunks}] bpb={rbpb:.6f} time={elapsed:.1f}s")
...
def main() -> None:
    ...
    if hasattr(base_model.smear, 'lookahead_scale'):
        scalar_params.append(base_model.smear.lookahead_scale)
    ...
    if master_process and args.wandb_project:
        ...
        _wandb = wandb.init(
            ...
            config={
                ...
                "grad_clip_norm": args.grad_clip_norm, "eval_stride": args.eval_stride,
                "bigram_vocab_size": args.bigram_vocab_size, "xsa_last_n": args.xsa_last_n,
    ...
    while True:
        ...
        if should_validate:
            ...
            if _wandb:
                _wandb.log({"val/loss": val_loss, "val/bpb": val_bpb,
                            "train/time_ms": training_time_ms}, step=step)
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        if last_step:
        ...
        if should_log_train:
            ...
            if _wandb:
                _wandb.log({"train/loss": train_loss.item(), "train/lr_scale": scale,
    ...
    if args.lookahead_smear and args.eval_passes > 1 and args.eval_stride > 0:
        ...
        for n_passes in range(2, args.eval_passes + 1):
            ...
            la_val_loss, la_val_bpb = eval_val_sliding_lookahead(
                args, eval_model, rank, world_size, device,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                stride=args.eval_stride,
                num_passes=n_passes,
            ...
            torch.cuda.synchronize()
            log0(
                f"final_lookahead_{n_passes}pass val_loss:{la_val_loss:.4f} val_bpb:{la_val_bpb:.4f} "
                f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_la):.0f}ms"
            )
            log0(f"final_lookahead_{n_passes}pass_exact val_loss:{la_val_loss:.8f} val_bpb:{la_val_bpb:.8f}")
            if _wandb:
                _wandb.log({f"final/lookahead_{n_passes}pass_bpb": la_val_bpb,
                            f"final/lookahead_{n_passes}pass_loss": la_val_loss})
    # Cache mixer evaluation
    if args.cache_enabled and args.eval_stride > 0:
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(f"cache_mixer val_loss:{cache_loss:.4f} val_bpb:{cache_bpb:.4f} "
             f"eval_time:{1000.0 * (time.perf_counter() - t_cache):.0f}ms")
        log0(f"cache_mixer_exact val_loss:{cache_loss:.8f} val_bpb:{cache_bpb:.8f}")
    # TTT evaluation
    ...
    if _wandb:
        _wandb.log({
            "final/val_bpb": sw_val_bpb if args.eval_stride > 0 and args.eval_stride < sw_seq_len else q_val_bpb,
            "final/val_loss": sw_val_loss if args.eval_stride > 0 and args.eval_stride < sw_seq_len else q_val_loss,
            "final/artifact_bytes": quant_file_bytes + code_bytes if master_process else 0,
            "final/train_steps": step,
            "final/train_time_ms": training_time_ms,
        })
        _wandb.finish()
    if distributed:
