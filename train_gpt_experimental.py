...
def xsa_correction(y: Tensor, v: Tensor, num_heads: int, num_kv_heads: int) -> Tensor:
    """Exclusive Self Attention: subtract each token's own value contribution.
    z_i = y_i - (y_i^T v_i) v_i / ||v_i||^2
    See https://arxiv.org/abs/2603.09078
    """
    if num_kv_heads != num_heads:
        v = v.repeat_interleave(num_heads // num_kv_heads, dim=1)
    dot = (y * v).sum(dim=-1, keepdim=True)
    v_norm_sq = v.square().sum(dim=-1, keepdim=True).clamp_min(1e-8)
    return y - (dot / v_norm_sq) * v


class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float, xsa: bool = True):
        super().__init__()
        if dim % num_heads != 0:
    ...
    def forward(self, x: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        if self.xsa:
            y = xsa_correction(y, v, self.num_heads, self.num_kv_heads)
        y = y.transpose(1, 2).contiguous().reshape(bsz, seqlen, dim)
        return self.proj(y), q, k


class ReuseSelfAttention(nn.Module):
    """Attention that reuses externally-provided Q, K — only has V projection + output projection."""
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, xsa: bool = True):
        super().__init__()
        self.num_heads = num_heads
        self.num_kv_heads = num_kv_heads
        self.head_dim = dim // num_heads
    ...
    def forward(self, x: Tensor, q: Tensor, k: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape
        v = self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        y = F.scaled_dot_product_attention(
            q, k, v, attn_mask=None, is_causal=True,
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float, rope_base: float, qk_gain_init: float, reuse_qk: bool = False, xsa: bool = True):
        super().__init__()
        self.reuse_qk = reuse_qk
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        if reuse_qk:
            self.attn = ReuseSelfAttention(dim, num_heads, num_kv_heads, xsa=xsa)
        else:
            self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, xsa=xsa)
        self.mlp = MLP(dim, mlp_mult)
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim))).float())

    def forward(self, x: Tensor, x0: Tensor, reuse_q: Tensor | None = None, reuse_k: Tensor | None = None) -> tuple[Tensor, Tensor | None, Tensor | None]:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        if self.reuse_qk:
            attn_out = self.attn(self.attn_norm(x), reuse_q, reuse_k)
            q, k = None, None
        else:
            attn_out, q, k = self.attn(self.attn_norm(x))
...
class GPT(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            is_decoder = i >= self.num_encoder_layers
            blocks.append(Block(
                model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                reuse_qk=(attn_reuse and is_decoder),
                xsa=xsa,
            ))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x0 = x
        skips: list[Tensor] = []
        saved_q: Tensor | None = None
        saved_k: Tensor | None = None
        for i in range(self.num_encoder_layers):
            x, q, k = self.blocks[i](x, x0)
            skips.append(x)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _, _ = self.blocks[self.num_encoder_layers + i](x, x0, reuse_q=saved_q, reuse_k=saved_k)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        attn_reuse=args.attn_reuse,
        xsa=args.xsa_enabled,
    ).to(device).bfloat16()
